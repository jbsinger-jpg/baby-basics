...
export default function BathRow({ bath, bathDataIsLoading, tabIndex, ml }) {
    ...
    const handleAddRating = async () => {
        let ratingSum = 0;
        const bathRef = await firestore.collection("bath");

        bathRef.doc(bath.id).collection("ratings").add({
            rating: Number(rating),
        });

        // update the data with the new rating
        const _total = (await bathRef.doc(bath.id).collection("ratings").get()).size;
        const sum = (await bathRef.doc(bath.id).collection("ratings").get()).docs.map(doc => doc.data().rating);
    ...
    useEffect(() => {
        ...
        (firestore.collection("bath").doc(bath.id).collection("ratings").get()).then(snapshot => {
                    ...
                    <VStack spacing="1" width={150} justifyContent="center" alignItems="start">
                        ...
                        {bath.description &&
                            <VStack alignItems="start">
                                ...
                                <Tag
                                    ...
                                    colorScheme='orange'
                                >
                                    <TagLabel>{bath.description}</TagLabel>
                                </Tag>
                            </VStack>
                        }
                        <Divider />
                        <VStack alignItems="start">
                            ...
                            <Tag
                                ...
                                colorScheme='gray'
                            >
                                <TagLabel>{"$" + bath.price?.toFixed(2)}</TagLabel>
                            </Tag>
                        </VStack>
                        <Divider />
                    </VStack>
