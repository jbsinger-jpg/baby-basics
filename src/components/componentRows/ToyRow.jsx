...
import { Box, Button, Card, CardBody, CardFooter, CardHeader, CircularProgress, Divider, HStack, Heading, Icon, Image, SkeletonCircle, SkeletonText, Stack, Tag, TagLabel, Text, Tooltip, VStack, useColorModeValue } from '@chakra-ui/react';
...
export default function ToyRow({ toy, toyDataIsLoading, tabIndex }) {
    ...
    const handleAddRating = async () => {
        let ratingSum = 0;
        const toyRef = await firestore.collection("toys");

        toyRef.doc(toy.id).collection("ratings").add({
            rating: Number(rating),
        });

        // update the data with the new rating
        const _total = (await toyRef.doc(toy.id).collection("ratings").get()).size;
        const sum = (await toyRef.doc(toy.id).collection("ratings").get()).docs.map(doc => doc.data().rating);
    ...
    useEffect(() => {
        ...
        (firestore.collection("toys").doc(toy.id).collection("ratings").get()).then(snapshot => {
    ...
    return (
        <VStack
            ...
            paddingBottom="10"
        >
            <SkeletonCircle size='10' isLoaded={!toyDataIsLoading} />
            <SkeletonText isLoaded={!toyDataIsLoading}>
